...
public:
	static vector<wstring> Enumerate()
	{
		...
		for (int n = 0; n < nDeviceCount; n++)
			...
			{
				size_t newsize = strlen(woc.szPname) + 1;
				wchar_t *wocSzPname = new wchar_t[newsize];
				size_t convertedChars = 0;
    			mbstowcs_s(&convertedChars, wocSzPname, newsize, woc.szPname, _TRUNCATE);
				sDevices.push_back(wocSzPname);
